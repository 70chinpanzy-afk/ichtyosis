...
class TrendFollowingStrategy:
    ...
    def generate_signal(self, df: pd.DataFrame,
                        daily_df: pd.DataFrame = None) -> "Signal | None":
        ...
        # Skip when ATR is abnormally high (>2x avg) or low (<0.3x avg)
        ...
        # If daily data is provided, check trend alignment
        mtf_bonus = 0.0
        mtf_note = ""
        if daily_df is not None and len(daily_df) > 50:
            daily_ema20 = calculate_ema(daily_df["close"], 20)
            daily_ema50 = calculate_ema(daily_df["close"], 50)
            daily_price = daily_df["close"].iloc[-1]

            daily_bullish = daily_price > daily_ema20.iloc[-1] > daily_ema50.iloc[-1]
            daily_bearish = daily_price < daily_ema20.iloc[-1] < daily_ema50.iloc[-1]

            if direction == "long" and daily_bullish:
                mtf_bonus = 0.15
                mtf_note = " | Daily ALIGNED ✓"
            elif direction == "short" and daily_bearish:
                mtf_bonus = 0.15
                mtf_note = " | Daily ALIGNED ✓"
            elif direction == "long" and daily_bearish:
                return None  # Reject: counter-trend
            elif direction == "short" and daily_bullish:
                return None  # Reject: counter-trend
        ...
        strength = max(0.1, min(strength + mtf_bonus, 1.0))
...
def run_all_strategies(df: pd.DataFrame,
                       ...
                       daily_df: pd.DataFrame = None) -> list[Signal]:
    ...
    for strat in strategies:
        # Pass daily_df for multi-timeframe confirmation if supported
        if hasattr(strat, 'generate_signal'):
            import inspect
            sig_params = inspect.signature(strat.generate_signal).parameters
            if 'daily_df' in sig_params:
                sig = strat.generate_signal(df, daily_df=daily_df)
            else:
